...
/* Creating a structure to store the required information
 * act -> activation
 * time_taken[27] -> stores the time for all the SYS_calls.
 * freq[27] -> stores the number of time each SYS_call is called
 * this structure is replicated for all the processes
 */ 
struct proc_track
{
	int act;
	unsigned long time_taken[27];
	int freq[27];
};

extern struct proc_track track_obj[NPROC];
//this global flag represents the start of collection of data. 
extern int flag_11;
